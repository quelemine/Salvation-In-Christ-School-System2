...
export default function TeacherPayrollPage() {
  ...
  return <div className="space-y-6">
    <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
      <div><p className="text-xs font-bold uppercase tracking-widest text-blue-700">Finance</p><h1 className="mt-1 text-3xl font-bold tracking-tight text-slate-900">Teacher payroll</h1><p className="mt-2 text-sm text-slate-500">Set salary structures, create monthly payroll, and track payment status.</p></div>
      <div className="flex flex-wrap gap-2"><button onClick={() => openStructure()} className="rounded-lg border border-slate-300 px-4 py-2.5 text-sm font-semibold text-slate-700 hover:bg-slate-50">+ Salary structure</button><button onClick={() => { setPayrollForm({ teacher_id: '', salary_structure_id: '', amount: '', currency: 'LRD', role_title: '', employment_type: 'self_contained', notes: '' }); setPayrollOpen(true); }} className="rounded-lg bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-blue-700">+ Create payroll</button></div>
    </div>
